...
from roadsense import local_eval
...
def _write_inputs(tmp_path: Path, *, tasks: list[str] | None = None) -> Path:
    ...
    if "segmentation" in selected_tasks:
        ...
        np.save(truth_mask, np.zeros((2, 20, 20), dtype=np.int64))
        np.save(prediction_mask, np.zeros((2, 20, 20), dtype=np.int64))
...
def test_local_segmentation_rejects_truth_prediction_shape_mismatch(tmp_path: Path) -> None:
    spec = _write_inputs(tmp_path, tasks=["segmentation"])
    mask = tmp_path / "truth-seq-a.npy"
    np.save(mask, np.zeros((2, 4, 4), dtype=np.int64))
    np.save(tmp_path / "prediction-seq-a.npy", np.zeros((2, 5, 4), dtype=np.int64))
    with pytest.raises(LocalEvaluationError, match="mask shapes do not match"):
        evaluate_local(spec)


def test_concat_masks_has_an_aggregate_memory_guard(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(local_eval, "MAX_MASK_ELEMENTS", 3)
    masks = {
        "a": np.zeros((2, 1, 1), dtype=np.int64),
        "b": np.zeros((2, 1, 1), dtype=np.int64),
    }
    with pytest.raises(LocalEvaluationError, match="aggregate segmentation masks"):
        local_eval._concat_masks(masks, ("a", "b"))
...
def test_local_spec_rejects_control_and_padded_sequence_values(tmp_path: Path) -> None:
    spec = _write_inputs(tmp_path)
    payload = json.loads(spec.read_text(encoding="utf-8"))
    payload["ground_truth"] = "truth\u0000.json"
    with pytest.raises(LocalEvaluationError, match="control"):
        load_local_spec(_write_json(tmp_path / "control.json", payload))

    payload = json.loads(spec.read_text(encoding="utf-8"))
    payload["split_sequences"]["development"] = [" seq-a"]
    with pytest.raises(LocalEvaluationError, match="surrounding whitespace"):
        load_local_spec(_write_json(tmp_path / "padded.json", payload))
...
def test_local_evaluation_canonicalizes_sequence_order(tmp_path: Path) -> None:
    spec = _write_inputs(tmp_path, tasks=["detection"])
    for bundle_name in ("truth.json", "prediction.json"):
        bundle_path = tmp_path / bundle_name
        payload = json.loads(bundle_path.read_text(encoding="utf-8"))
        payload["sequences"].append({"sequence_id": "seq-b", "frames": [_frame(index=0)]})
        bundle_path.write_text(json.dumps(payload), encoding="utf-8")
    spec_payload = json.loads(spec.read_text(encoding="utf-8"))
    spec_payload["split_sequences"]["development"] = ["seq-b", "seq-a"]
    spec_payload["split_sequences"]["final"] = ["seq-c"]
    spec.write_text(json.dumps(spec_payload), encoding="utf-8")

    report = evaluate_local(spec)

    assert report["details"]["sequence_ids"] == ["seq-a", "seq-b"]  # type: ignore[index]


def test_local_evaluation_rejects_input_replacement_before_report(tmp_path: Path) -> None:
    spec = _write_inputs(tmp_path, tasks=["detection"])
    case = local_eval.load_local_evaluation(spec)
    ...
    with pytest.raises(LocalEvaluationError, match="changed during evaluation"):
        local_eval._assert_input_hashes_stable(case)


def test_local_evaluation_rejects_input_replacement_during_parsing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    spec = _write_inputs(tmp_path, tasks=["detection"])
    original_loader = local_eval._load_sequence_bundle

    def load_then_replace(path: Path, *, role: str):
        result = original_loader(path, role=role)
        if role == "ground_truth":
            path.write_text(path.read_text(encoding="utf-8") + "\n", encoding="utf-8")
        return result

    monkeypatch.setattr(local_eval, "_load_sequence_bundle", load_then_replace)
    with pytest.raises(LocalEvaluationError, match="ground_truth bundle changed during parsing"):
        local_eval.load_local_evaluation(spec)
...
def test_local_report_binds_verified_model_artifact(tmp_path: Path) -> None:
    ...
    lock_path = artifact_root / "runtime.lock"
    lock_bytes = b"runtime-lock-v1\n"
    ...
    manifest_payload = {
        ...
        "dependency_lock_path": "runtime.lock",
    ...
    assert "model_dependency_lock" in report["details"]["input_hashes"]  # type: ignore[index]
    assert report["report_id"] == compute_report_id(report)

    lock_path.write_bytes(lock_bytes + b"tampered")
    with pytest.raises(LocalEvaluationError, match="model dependency lock"):
        local_eval._assert_input_hashes_stable(case)
...
def test_cli_rejects_output_overwriting_local_input(tmp_path: Path, capsys) -> None:
    spec = _write_inputs(tmp_path, tasks=["detection"])
    prediction_path = tmp_path / "prediction.json"
    original = prediction_path.read_bytes()

    assert main(["evaluate-local", str(spec), "--output", str(prediction_path)]) == 2

    assert prediction_path.read_bytes() == original
